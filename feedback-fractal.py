...
def run():
    ...
    while key_pressed != 113:  # press 'q' to exit
        ...
        m_1 = cv2.getRotationMatrix2D(
            (WIDTH / 2 + c1 * 20, HEIGHT / 2 + c2 * 20), angle * c1, 0.8 * c1
        )
        m_1.resize((3, 3))
        m_1[2, 2] = 1
        m_2 = np.array(
            [[1.0, 0, WIDTH * 0.3 * c1], [0, 1.0, HEIGHT * 0.2 * c4], [0, 0, 1]]
        )
        m_3 = np.dot(m_2, m_1)
        ...
        m_1 = cv2.getRotationMatrix2D(
            (WIDTH / 2 + c3 * 20, HEIGHT / 2 + c2 * 20), 125 + angle * c2, 0.6 * c2
        )
        m_1.resize((3, 3))
        m_1[2, 2] = 1
        m_2 = np.array(
            [[1.0, 0, -WIDTH * 0.4 * c3], [0, 1.0, -HEIGHT * 0.1 * c4], [0, 0, 1]]
        )
        m_3 = np.dot(m_2, m_1)
        # assume a square template image
        ...
        m_1 = cv2.getRotationMatrix2D(
            (WIDTH / 2 + c2 * 20, HEIGHT / 2 + c4 * 20), 25 + angle * c4, 0.3 * c3
        )
        m_1.resize((3, 3))
        m_1[2, 2] = 1
        m_2 = np.array(
            [[1.0, 0, -WIDTH * 0.5 * c3], [0, 1.0, -HEIGHT * 0.4 * c4], [0, 0, 1]]
        )
        m_3 = np.dot(m_2, m_1)
        # assume a square template image
        ...
        img_current = cv2.addWeighted(
            img_current, 0.9, cv2.applyColorMap(img_current, cv2.COLORMAP_HSV), 0.1, 0
        )
